Function Set-VPLEXDatastorePreferedPath {
  ...
  Param(
    ...
    Begin {
        $VPLEXPorts = Get-VPLEXPortFromSeed -Seed $Seed -Director $Director | Where-Object {($_.IOModule -Match 0) -and ($Port -contains $_.Port)} #Get VPLEX's Ports from the seed provided
    }
